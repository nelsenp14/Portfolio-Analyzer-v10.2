...
var computed=useMemo(function(){var tvv=0,tcc=0,dii=0;var wins=0,total=0;var poss=validH.map(function(x){var tk=cT(x.ticker),e=enriched[tk]||{};var price=e.currentPrice||parseFloat(x.avgCost)||0,shares=parseFloat(x.shares)||0,ac=parseFloat(x.avgCost)||0;if(!ac&&price>0)ac=price;var v=price*shares,cost=ac*shares;var divPerShare=e.annualDividendPerShare||0;if(!divPerShare&&e.dividendYield>0&&price>0)divPerShare=price*(e.dividendYield/100);var div=divPerShare*shares;tvv+=v;tcc+=cost;dii+=div;if(e.currentPrice>0){total++;if(price>=ac)wins++;}return Object.assign({},x,e,{ticker:tk,price:price,shares:shares,avgCost:ac,value:v,cost:cost,divIncome:div});});var peN=0,peD=0;poss.forEach(function(p){if(p.pe>0&&p.pe<500){peN+=p.pe*p.value;peD+=p.value;}});var wr=total>0?Math.round((wins/total)*100):0;return{pos:poss,tv:tvv,tc:tcc,di:dii,dy:tvv>0?(dii/tvv)*100:0,wpe:peD>0?peN/peD:0,winRate:wr,winCount:wins,totalCount:total};},[validH,enriched]);
...
<div style={{display:"flex",alignItems:"center",gap:8}}><input value={hh.ticker} onChange={function(ev){setHoldings(function(hs){return hs.map(function(x){return x.id===hh.id?Object.assign({},x,{ticker:ev.target.value.toUpperCase()}):x;});});}} placeholder="AAPL" style={Object.assign({},inp,{width:60,fontWeight:700,color:"#60a5fa",fontSize:15,padding:"4px 6px"})}/><div style={{fontSize:10,color:"#94a3b8",maxWidth:90,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{isF?"...":e.companyName||""}</div></div>
<div style={{display:"flex",gap:4}}><button onClick={function(){if(tk&&!isF)liveOne(tk);}} className="refBtn" style={{background:"#0f172a",color:"#3b82f6",border:"1px solid #1e3a5f",borderRadius:5,padding:"4px 6px",cursor:"pointer",fontSize:10}}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg></button><button onClick={function(){setHoldings(function(hs){return hs.filter(function(x){return x.id!==hh.id;});});}} className="xBtn" style={{background:"#0f172a",color:"#3b82f6",border:"1px solid #1e3a5f",borderRadius:5,padding:"4px 6px",cursor:"pointer",fontSize:10}}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>
</div>
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:6}}>
<div><div style={{fontSize:9,color:"#64748b"}}>Shares</div><input value={hh.shares} onChange={function(ev){setHoldings(function(hs){return hs.map(function(x){return x.id===hh.id?Object.assign({},x,{shares:ev.target.value}):x;});});}} style={Object.assign({},inp,{fontSize:12,padding:"4px 6px"})}/></div>
<div><div style={{fontSize:9,color:"#64748b"}}>Cost</div><input value={hh.avgCost} onChange={function(ev){setHoldings(function(hs){return hs.map(function(x){return x.id===hh.id?Object.assign({},x,{avgCost:ev.target.value}):x;});});}} style={Object.assign({},inp,{fontSize:12,padding:"4px 6px"})}/></div>
<div><div style={{fontSize:9,color:"#64748b"}}>Price</div><div style={{fontSize:12,fontWeight:500,color:"#fff",padding:"4px 6px"}}>{price?"$"+price.toFixed(2):"--"}</div></div>
</div>
<div style={{display:"flex",justifyContent:"space-between",fontSize:11}}>
<span style={{color:"#94a3b8"}}>{v2?fmt(Math.round(v2)):"--"}{wt?" ("+wt.toFixed(1)+"%)":""}</span>
<span style={{color:gl2>=0?"#22c55e":"#ef4444",fontWeight:600}}>{price&&hh.avgCost?(gl2>=0?"+":"")+gl2.toFixed(1)+"%":"--"}</span>
{e.sector?<span style={{fontSize:9,color:"#64748b"}}>{e.sector}</span>:null}
</div>
</div>;})}</div>
